...
def initInitD():
    ...
    script = mw.getRunDir() + '/scripts/init.d/ng-panel.tpl'
    script_bin = mw.getRunDir() + '/scripts/init.d/ng-panel'
    ...
    if os.path.exists("/etc/init.d"):
        initd_bin = '/etc/init.d/ng-panel'
        if not os.path.exists(initd_bin):
            import shutil
            shutil.copyfile(script_bin, initd_bin)
            mw.execShell('chmod +x ' + initd_bin)
        # 加入自启动
        mw.execShell('which chkconfig && chkconfig --add ng-panel')
        mw.execShell('which update-rc.d && update-rc.d -f ng-panel defaults')
