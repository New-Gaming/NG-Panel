...
def initSlaveStatus(version=''):
    ...
    try:
        ...
        cmd = 'cd /www/server/ng-panel && python3 plugins/mariadb/index.py get_master_rep_slave_user_cmd {"username":"' + db_user + '","db":""}'
...
def doFullSync(version=''):
    ...
    dbname = args['db']
    cmd = "cd /www/server/ng-panel && python3 plugins/mariadb/index.py dump_mysql_data {\"db\":'" + dbname + "'}"
    print(cmd)
    stdin, stdout, stderr = ssh.exec_command(cmd)
    result = stdout.read()
    result = result.decode('utf-8')
