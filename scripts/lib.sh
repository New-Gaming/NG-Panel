...
cd /www/server/ng-panel && pip3 install -r /www/server/ng-panel/requirements.txt
...
if [ ! -f /www/server/ng-panel/bin/activate ];then
    cd /www/server/ng-panel && python3 -m venv .
    cd /www/server/ng-panel && source /www/server/ng-panel/bin/activate
else
    cd /www/server/ng-panel && source /www/server/ng-panel/bin/activate
fi
...
pip install --upgrade pip
pip3 install --upgrade setuptools
pip3 install -r /www/server/ng-panel/requirements.txt
