...
class system_api:
    ...
    def getAllInfo(self, get):
        ...
        data['panel_status'] = not os.path.exists(
            '/www/server/ng-panel/data/close.pl')
        import firewalls
    ...
    def clearOther(self):
        clearPath = [
            {'path': '/www/server/ng-panel', 'find': 'testDisk_'},
            {'path': '/www/wwwlogs', 'find': 'log'},
            {'path': '/tmp', 'find': 'panelBoot.pl'},
            {'path': '/www/server/ng-panel/install', 'find': '.rpm'}
        ]
    ...
    def getServerInfo(self):
        ...
        import ssl
        upAddr = 'https://api.github.com/repos/New-Gaming/NG-Panel/releases/latest'
        try:
    ...
    def updateServer(self, stype, version=''):
        ...
        try:
            ...
            if stype == 'update':
                ...
                newUrl = "https://github.com/New-Gaming/NG-Panel/archive/refs/tags/" + version + ".zip"

                dist_mw = toPath + '/ng.zip'
                if not os.path.exists(dist_mw):
                    mw.execShell(
                        'wget --no-check-certificate -O ' + dist_mw + ' ' + newUrl)

                dist_to = toPath + "/ng-panel-" + version
                if not os.path.exists(dist_to):
                    os.system('unzip -o ' + toPath +
                              '/mw.zip' + ' -d ' + toPath)

                cmd_cp = 'cp -rf ' + toPath + '/ng-panel-' + \
                    version + '/* ' + mw.getServerDir() + '/ng-panel'
                mw.execShell(cmd_cp)

                mw.execShell('rm -rf ' + toPath + '/ng-panel-' + version)
                mw.execShell('rm -rf ' + toPath + '/ng.zip')
    ...
    def repPanel(self, get):
        vp = ''
        if mw.readFile('/www/server/ng-panel/class/common.py').find('checkSafe') != -1:
            vp = '_pro'
