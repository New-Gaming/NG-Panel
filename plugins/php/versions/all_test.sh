...
# cd /www/server/ng-panel/plugins/php/versions && /bin/bash all_test.sh

# cd /www/server/ng-panel/plugins/php && bash install.sh install 55
# cd /www/server/ng-panel/plugins/php/versions/common && bash gd.sh install 73
# cd /www/server/ng-panel/plugins/php/versions/common && bash swoole.sh install 54
...
for PHP_VER in ${PHP_VER_LIST[@]}; do
	...
	if [ ! -d  /www/server/php/${PHP_VER} ];then
		cd /www/server/ng-panel/plugins/php && bash install.sh install ${PHP_VER}
	fi
	echo "php${PHP_VER} -- end"
done
